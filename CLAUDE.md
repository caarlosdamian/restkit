...
⚠️ **An uploaded `customIconUrl` OVERRIDES `stampIcon`** — `lib/strip-render.ts` checks it first and never looks at the catalogue when it is set. That made an upload permanent: there was no control to remove it, and clicking a catalogue icon only changed a value the renderer ignores **while the grid highlighted the new choice**, so the picker said one thing and the card printed another. Now picking from the catalogue clears the upload, the grid shows nothing selected while an upload is in force, and the uploaded icon is shown with a "Quitar" button. `e2e/stamp-icon.spec.ts` holds both exits. Same shape as the geofence pin: **anything that overrides something else needs a way back, and the override has to be visible while it is winning.**

⚠️ **Upload formats live in `lib/upload-limits.ts`, not in the pickers.** The route and three `accept` attributes each declared their own and had drifted — the photo picker said `image/*`, which offers every HEIC on the machine (and every photo an iPhone takes is HEIC) for the server to then refuse. A picker may be NARROWER than the server, never wider. A stamp icon is PNG/SVG only because it is drawn straight onto the ground colour and a JPEG ships a white box around the mark; the form says so, since nothing on screen would otherwise explain it.
...
⚠️ **The geofence is not a notification, and that is why it cannot be dismissed.** `locations[]` produces a lock-screen pass **suggestion** — a live state iOS maintains while it believes the pass is useful where you are, cleared when you leave. Swiping does not end the condition that put it there, so an owner testing inside their own restaurant sees it permanently; that is correct behaviour, not a bug. Two things are ours: `relevantText` is plain text (Apple substitutes nothing into it, unlike `changeMessage`), so `{progreso}` is filled in by `fillTokens()` on our side — one token for the owner to learn, not two rules; and `maxDistance` is the only lever over how insistent it feels, since leaving it unset hands iOS a ~100m bubble, half a street for a café on a corner. ⚠️ **Setting the pin used to be `navigator.geolocation` and nothing else**, which pins the card to wherever the browser is standing — and owners do admin from home, so the geofence landed on their house and the card never appeared at the restaurant. It failed in total silence: nothing in the product can report a card that is not showing. `lib/geo-pin.ts` now parses a pasted Google/Apple Maps link or a bare pair, `mapsUrl()` gives a "Verificar en el mapa" link (coordinates alone tell an owner nothing about whether the pin is on their door), and there is a "Quitar ubicación" button — `passLocations()` only bails when `latitude` is null, so before that a wrong pin shipped forever. ⚠️ **A Google place URL carries the coordinates twice**: `@lat,lng` is the map camera and `!3d/!4d` is the pin. They differ whenever the view was dragged before copying, so the pin wins. A short `goo.gl` link is its own error, because the coordinates are behind a redirect the browser cannot follow. The geolocation failure paths now say which one happened; all three used to be indistinguishable from the button doing nothing.

`passLocations()` in `lib/apple-pass.ts` is extracted precisely so this is testable without signing certs, and ⚠️ **an absent radius is not `0`** — a `maxDistance` of zero is a geofence nothing can enter. Google's `locations[]` takes neither field.
...
- ⚠️ **`isActive` — "eliminar cliente" ARCHIVES, it does not delete.** A customer is the only explanation for their own `Visit` rows, and `Visit` is append-only; hard-deleting one would leave "premios entregados" and every past cash-up describing somebody who is not there, orphan `Order.customerId` on paid tickets, and silently wipe a cashback balance the business still owed. Archiving takes them out of **the roster, the till lookup and `/scan`** and leaves everything else alone. Reversible from the dashboard's "Eliminados" tab — which is not a nicety: without it, archiving is not actually reversible.
  - Queries filter `isActive: { $ne: false }`, **never `=== true`** — every row that predates the field has no `isActive` at all and must still count as active.
  - The rule for what respects it: a figure counted in **people** (roster, top customers, signups, `cashbackLiability`, the CSV) drops archived customers; a figure counted in **visits** is history and is untouched. The balance stays on the row, so restoring brings it back exactly as it was.
  - An archived card scanned at `/scan` **404s, never "disabled"** — same reasoning as a token from another business.
  - ⚠️ The `{businessId, phone}` unique deliberately still covers archived rows, so re-enrolling that number at `/j/[slug]` answers `ALREADY_ENROLLED` rather than minting a second card for one human. Restoring them is the way back.
...
- `PATCH /api/customers/[customerId]` — Update customer (name/email/phone), or `{ isActive: true }` to restore an archived one. **409 `DUPLICATE_CONTACT`** when the phone or email already belongs to another customer of the same business — that collision is an ordinary mistake, and as a 500 it read as "the app is broken" and the owner retried forever.
- `DELETE /api/customers/[customerId]` — **archives** (OWNER/ADMIN). See the ⚠️ on `Customer.isActive`: this is not a delete. 404 for another business's customer, 403 for STAFF.
...
- ⚠️ **The tier has two sources, and the order matters.** `applySubscription` takes the plan from the subscription's **metadata** when present, and otherwise **derives it from the price id** via `planForPriceId()` (the exact inverse of `requirePriceId`, reading the same six `STRIPE_PRICE_*` vars). An unrecognised price — a one-off, a grandfathered deal — resolves to nothing and the plan is left exactly as it was.
  - This is what makes a subscription created **anywhere** land on the right tier: the Stripe dashboard, the API, a script. It reads the price id, which every subscription carries whether or not anyone remembered metadata.
  - Before the fallback existed, a dashboard-created subscription granted access but left `Business.subscription.plan` at whatever signup defaulted to (`pro`): a business sold Lite ran as Profesional while paying $249, silently, and the only way back was editing metadata by hand. **Nothing needs repairing now, and nothing has to be remembered.**
  - Metadata still wins when both are present, so swapping a price for a grandfathered one cannot silently re-tier a business whose plan was set deliberately. `tests/unit/stripe-price-map.test.ts` holds the round trip; `tests/integration/billing-manual-sub.test.ts` holds all four combinations.
- **Selling a plan outside Checkout** — paid by transfer, agreed on the phone: `npm run grant:sub -- --business <slug|correo|nombre> --plan basic --period monthly` (dry run; add `--apply`). Optional, not required — the dashboard works too. It exists because it checks what the dashboard cannot: that the price matches the plan intended, that the business is not already paying, that the billing mode will not leave the subscription unpaid, and that the webhook actually arrived. Both writers build metadata through `subscriptionMetadata()` in `lib/billing-metadata.ts`.
  - ⚠️ The script **never writes to Mongo** — `applyStripeEvent` stays the only thing that moves a business onto a plan. After creating the subscription it watches for the webhook to land and fails loudly if it does not, so an unwired `POST /api/stripe/webhook` surfaces then rather than at the first renewal.
  - ⚠️ It invoices by default (`collection_method: 'send_invoice'`). `--auto` charges a saved card, but with no card on file Stripe leaves the subscription `incomplete`, which `mapStatus` turns into `past_due` — the business would be **blocked** by the thing meant to give them access.
  - `businessId` in the metadata means it works for a business that never opened `/dashboard/billing`; without it, `findBusinessId` falls back to the stored `stripeCustomerId`, which only exists once they have clicked a plan.
  - ⚠️ **Scripts under `scripts/*.mts` run straight through `node`** (built-in type stripping, no build step, no extra dependency). That is why `tsconfig.json` sets `allowImportingTsExtensions` and why `StripePriceMisconfiguredError` spells its field out instead of using a constructor parameter property — strip-only mode refuses those.
...
- **The hero's second CTA is a demo, not a link to the modules** (`components/landing/CardDemo.tsx`, data in `lib/card-demo.ts`). Five steps in a modal: pick one of two example businesses (plus an empty "El tuyo" slot that deliberately cannot be filled), pick sellos or cashback, see the settings **locked**, see the push on a lock screen, then the finished card and "Comenzar ahora" → `/registro`.
  - ⚠️ **The last step is the real `LoyaltyCard`, from a real `loyaltyConfig()`** — the same component `/c/[token]` serves a customer. A mockup would drift from the product the first time either changed, and "this is the thing you get" is the entire argument.
  - **Only two things are choosable, and both change what a card fundamentally IS**: which business, and which mechanic. Everything else is shown read-only with a lock. A builder that lets a stranger type their restaurant's name before signing up collects nothing and teaches nothing.
  - `LoyaltyCard` is safe in a client bundle — its only model import is `import type`, and `lib/loyalty`, `lib/card-colors` and `lib/stamp-icons` are all pure. ⚠️ `lib/strip-render.ts` is NOT: it imports sharp.
  - `tests/unit/card-demo.test.ts` holds the fixed data to the same rules a real business must satisfy (icon exists, accent clears 4.5:1, the card sits partway to its reward); `e2e/card-demo.spec.ts` walks the five steps.
...
- **Activos / Eliminados** tabs (`?estado=archivados`); edit and delete per row, restore on the archived tab
- RecordVisitButton per customer
- Links to `/dashboard/customers/[id]` for details
- `ArchiveCustomerButton` spells out an outstanding balance or unclaimed reward **before** the confirm — archiving puts that customer out of reach of the till lookup, so it is the one thing an owner must not discover afterwards. The confirm also says the history is kept, because "eliminar" implies otherwise.
- ⚠️ The row used to be one big `<Link>`; action buttons cannot nest inside an anchor, so the row is a flex with the link and the actions as siblings.
...
17. **The stamp icon catalogue has 71 icons**, not the 100 Loyalia advertises. Structure supports more; it's a matter of picking and thickening more lucide paths. ⚠️ Three of them — `padel`, `football` and `tennis` — are **drawn for RestKit, not lifted from lucide**, which ships no racket and no soccer or tennis ball at the installed version (it does have `volleyball`, `sport-shoe`, `person-standing`, `waves` and `mountain`, all used as-is — check before drawing). Same contract as the rest (one 24×24 `d`, stroke only, `fill="none"`), but the lesson that produced them is worth keeping: at `STAMP_STROKE` 2.6 **interior detail collapses**. A racket head with a 2×2 string grid filled in solid, and an upright head of any kind reads as a balloon or a magnifying glass — tilting the head and angling the handle is what makes the silhouette a racket before any detail resolves. Render a candidate at 24px against the filled badge before committing it, the way `tests/unit/loyalty.test.ts` renders a real strip. ⚠️ **A basketball was attempted and abandoned** — six constructions were rendered and every one reads as a globe or a ⊘ at stamp size, because what makes a basketball is its seam curvature and that needs finer strokes than 2.6. Shipping it would have meant a globe labelled "Básquetbol"; if it is wanted, it needs a different treatment, not another arc.
